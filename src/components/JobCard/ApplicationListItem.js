  ...
  return (
    <Item>
      ...
      <Item.Content>
        ...
        <Item.Meta>
          <span>0 завершённых проектов, 0.0 рейтинг, 0% заказчиков рекомендуют</span>
        </Item.Meta>
        <Item.Description>{application.comment}</Item.Description>
        <Item.Extra>
          <Button size='tiny' floated='right'>Чат</Button>

          {contract
            ? (
              <Link href="/contracts/[id]" as={`/contracts/${contract.id}`} passHref>
